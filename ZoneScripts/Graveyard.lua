...
function SpawnObjectsForZaddarQuest4(Zone, Spawn)
	if HasQuest(Spawn, ZADDAR_QUEST_4) then
		if GetQuestStep(Spawn, ZADDAR_QUEST_4) == 2 then
			WardParticle = GetSpawn(Spawn, 1250125)
			if WardParticle == nil then
				WardParticle = SpawnMob(GetZone(Spawn), 1250125, false, 58.588, -2.72359, 98.4903, 339.984)
				Despawn(WardParticle, 30000)
			end
		elseif GetQuestStep(Spawn, ZADDAR_QUEST_4) == 3 then
			Relic = GetSpawn(Spawn, 1250107)
			if Relic == nil then
				Relic = SpawnMob(GetZone(Spawn), 1250107, false, 57.9408, -2.75514, 98.8084, 339.984)
				Despawn(Relic, 30000)
			end
		end
	end
